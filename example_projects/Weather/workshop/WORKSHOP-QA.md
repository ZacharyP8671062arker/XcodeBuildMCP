...
**Q: Why didn't the agent catch the broken-units bug from reading code?**
- It might have, on a careful read. But the demo point is: the agent's first move when asked "make sure the settings work" should be to *test* the app, not just stare at the source. That's the loop we want — code + test + verify, not code + assume.
...
- The bug we just hit — wrong city in the precipitation card — never crashed. Sentry wouldn't see it. You only catch this by stepping into the code with real data.

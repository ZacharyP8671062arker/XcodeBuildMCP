...
describe('device workflow (flowdeck)', () => {
  ...
  describe('build', () => {
    it.runIf(deviceId)('success', () => {
      const result = harness.run([
        'build', '-w', WORKSPACE, '-s', 'CalculatorApp', '-D', deviceId!,
      ]);
    ...
    it.runIf(deviceId)('error - wrong scheme', () => {
      const result = harness.run([
        'build', '-w', WORKSPACE, '-s', 'NONEXISTENT', '-D', deviceId!,
      ]);
  ...
  describe('build-and-run', () => {
    it.runIf(deviceId)('success', () => {
      const result = harness.run([
        'run', '-w', WORKSPACE, '-s', 'CalculatorApp', '-D', deviceId!,
      ]);
  ...
  describe('install', () => {
    it('error - invalid app path', () => {
      const result = harness.run([
        'device', 'install', '00000000-0000-0000-0000-000000000000', '/tmp/nonexistent.app',
      ]);
    ...
    it.runIf(deviceId)('success', () => {
      const result = harness.run([
        'device', 'install', deviceId!, '/tmp/nonexistent-device.app',
      ]);
  ...
  describe('launch', () => {
    it('error - invalid bundle', () => {
      const result = harness.run([
        'device', 'launch', '00000000-0000-0000-0000-000000000000', 'com.nonexistent.app',
      ]);
    ...
    it.runIf(deviceId)('success', () => {
      const result = harness.run([
        'device', 'launch', deviceId!, BUNDLE_ID,
      ]);
  ...
  describe('test', () => {
    it.runIf(deviceId)('success - targeted passing test', () => {
      const result = harness.run([
        'test', '-w', WORKSPACE, '-s', 'CalculatorApp', '-D', deviceId!,
        '--only', 'CalculatorAppTests/CalculatorAppTests/testAddition',
      ]);
      writeFlowdeckFixture(__filename, 'test--success', result.text);
    }, 300_000);

    it.runIf(deviceId)('failure - intentional test failure', () => {
      const result = harness.run([
        'test', '-w', WORKSPACE, '-s', 'CalculatorApp', '-D', deviceId!,
      ]);
